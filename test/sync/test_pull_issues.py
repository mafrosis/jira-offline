...
def test_pull_issues__calls_pull_single_project_for_each_project(mock_pull_single_project, mock_jira, project, project2):
    ...
    assert mock_pull_single_project.call_args_list[0][1] ==  {'force': True, 'verbose': False}
    assert mock_pull_single_project.call_args_list[1][0] ==  (project2,)
    assert mock_pull_single_project.call_args_list[1][1] ==  {'force': True, 'verbose': False}
...
def test_pull_issues__pulls_only_specified_projects(mock_pull_single_project, mock_jira, project):
    ...
    '''
    with mock.patch('jira_offline.sync.jira', mock_jira):
        pull_issues(projects={'TEST'}, force=True, verbose=False)

    mock_pull_single_project.assert_called_once_with(project, force=True, verbose=False)
...
def test_pull_single_project__calls_load_issues_when_self_empty(mock_tqdm, mock_api_get, mock_jiraapi_object_to_issue, mock_jira, project):
    ...
    with mock.patch('jira_offline.sync.jira', mock_jira):
        pull_single_project(project, force=False, verbose=False)
...
def test_pull_single_project__last_updated_set_AND_force_set_causes_filter_from_waaay_back(mock_tqdm, mock_api_get, mock_jiraapi_object_to_issue, mock_jira, project):
    ...
    with mock.patch('jira_offline.sync.jira', mock_jira):
        pull_single_project(project, force=True, verbose=False)
