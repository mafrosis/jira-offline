...
def test_create__create_issue__adds_issue_to_self_and_calls_write_issues(mock_jira):
    '''
    Ensure create_issue() adds the new Issue to self, and writes the issue cache
    '''
    offline_issue = create_issue(mock_jira, 'TEST', 'Story', 'This is a summary')

    assert mock_jira[offline_issue.key]
    assert mock_jira.write_issues.called


def test_create__create_issue__mandatory_fields_are_set_in_new_issue(mock_jira):
    '''
...
def test_create__create_issue__creates_issues_with_unspecified_status(mock_jira):
    '''
...
def test_create__create_issue__error_on_existing_summary_for_same_project(mock_jira):
    '''
    ...
    with pytest.raises(SummaryAlreadyExists):
        create_issue(mock_jira, 'TEST', 'Story', mock_jira['issue1'].summary)


def test_create__create_issue__NO_error_on_existing_summary_for_different_project(mock_jira):
    '''
...
def test_create__create_issue__raises_exception_when_passed_an_unknown_epic_ref(mock_jira):
    '''
    ...
    with pytest.raises(EpicNotFound):
        create_issue(mock_jira, 'TEST', 'Story', 'This is summary', epic_ref='Nothing')
...
def test_create__create_issue__issue_is_mapped_to_existing_epic_summary(mock_jira, epic_ref_value):
    '''
    ...
    new_issue = create_issue(mock_jira, 'TEST', 'Story', 'This is summary', epic_ref=epic_ref_value)
